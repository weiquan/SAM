...
class gem ():
    """docstring for gem """
    @staticmethod
    def convertCigar0(cigar):
        string_cigar = ''
        start = 0
        end = 0
        n = 0
        for i, c in enumerate(cigar):#to be fixed
            if c == '>':
                string_cigar += str(n)+'M'
            elif  c.isalpha():
                n += 1
            elif c.isdigit():
                end = i
                n = int(cigar[start:end+1])

        return string_cigar
    def convertCigar1(cigar):
    ...
    def printHitSAM(self):
        ...
        if self.is_pe and self.properPaired:
            ...
            cigar0 = gem.convertCigar(self.hit0[0].cigar)
            cigar1 = gem.convertCigar(self.hit1[0].cigar)
            print '{0}\t{1}\t{2}\t{3}\t255\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}'.format(
                    self.qname, flag0, self.hit0[0].chrom, self.hit0[0].pos, cigar0, 
                    self.hit0[0].mate.chrom, self.hit0[0].mate.pos, isize0, self.seq0, self.qual0, xa0)
            print '{0}\t{1}\t{2}\t{3}\t255\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}'.format(
                    self.qname, flag1, self.hit1[0].chrom, self.hit1[0].pos, cigar1, 
